...
def write_markdown(quote: RenovationQuote, output_path: Path) -> None:
    ...
    lines.append(
        f"timeline: {meta.timeline_weeks_min or '?'}-{meta.timeline_weeks_max or '?'} hét"
    )
    ...
    for phase in phases:
        ...
        if main_items:
            ...
            for item in main_items:
                lines.append(
                    f"| {item.name} | {format_huf(item.labor_cost)} | {format_huf(item.material_cost)} | {format_huf(item.total_cost)} |"
                )
            lines.append("")
        ...
        if alt_items:
            ...
            for item in alt_items:
                lines.append(
                    f"| {item.name} (v{item.version}) | {format_huf(item.labor_cost)} | {format_huf(item.material_cost)} | {format_huf(item.total_cost)} |"
                )
            lines.append("")

    # Detailed notes for items that have them
    all_items_with_notes = [
        li for li in quote.line_items + quote.alternatives if li.notes
    ]
