...
SECTION_KEYWORDS = {
    ...
    "buyer_purchases": [
        "tulajdonos vásárolja",
        "amit a tulajdonos vásárol",
...
def classify_rows(rows: List[tuple], col_map: ColumnMap) -> List[ClassifiedRow]:
    ...
    for idx, row in enumerate(rows):
        ...
        is_phase_label = any(
            kw in work_text_lower for kw in SECTION_KEYWORDS["phase_label"]
        )
        if is_phase_label:
        ...
        if (
            work_text
            and labor_val is None
            and material_val is None
