...
from .column_detector import detect_style_and_columns
...
def parse_quote(xlsx_path: Path) -> RenovationQuote:
    ...
    for cr in classified_rows:
        ...
        if cr.row_type == "line_item":
            work_name = (
                ...
                if row[col_map.work_name] is not None
                else ""
            )
            if not work_name:
                continue
            ...
            material = (
                parse_huf(row[col_map.material])
                if col_map.material is not None
                else None
            )
            ...
            notes = (
                str(row[col_map.notes]).strip()
                if col_map.notes is not None and row[col_map.notes] is not None
                else None
            )
            ...
            item = LineItem(
                ...
                total_cost=total or ((labor or 0) + (material or 0))
                if (labor or material)
                else total,
                notes=notes,
                section=cr.section,
                version=cr.version,
    ...
    if grand_total == 0 and grand_total_from_text["total"]:
        ...
        if abs(grand_total - grand_total_from_text["total"]) > 1000:
            warnings.append(
                f"Grand total mismatch: calculated {grand_total}, text summary {grand_total_from_text['total']}"
            )
    ...
    full_text = "\n".join(
        [" ".join([str(c) for c in r if c is not None]) for r in rows]
    )
