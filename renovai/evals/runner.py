...
class EvalRunner:
    ...
    def run_example(self, example: EvalExample) -> EvalResult:
        ...
        matches = [
            kw for kw in example.expected_answer_keywords if kw.lower() in answer
        ]
        kw_score = (
            len(matches) / len(example.expected_answer_keywords)
            if example.expected_answer_keywords
            else 1.0
        )
        ...
        found_sources = [
            s
            for s in example.expected_sources
            if any(s in actual for actual in sources)
        ]
        source_recall = (
            len(found_sources) / len(example.expected_sources)
...
def display_report(report: EvalSuiteReport):
    ...
    console.print(
        f"Avg Keyword Match: [green]{report.avg_keyword_score * 100:.1f}%[/green]"
    )
    console.print(
        f"Avg Source Recall: [green]{report.avg_source_recall * 100:.1f}%[/green]"
    )
