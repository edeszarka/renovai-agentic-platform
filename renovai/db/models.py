...
class Quote(Base):
    ...
    renovation_completeness: Mapped[
        Optional[str]
    ]  # "reszleges" | "komplett" | None (captured, not yet weighted)
    ...
    not_included: Mapped[List["NotIncluded"]] = relationship(
        back_populates="quote", cascade="all, delete-orphan"
    )
    buyer_purchases: Mapped[List["BuyerPurchase"]] = relationship(
        back_populates="quote", cascade="all, delete-orphan"
    )
    general_notes: Mapped[List["GeneralNote"]] = relationship(
        back_populates="quote", cascade="all, delete-orphan"
    )
    vectors: Mapped[List["QuoteVector"]] = relationship(
        back_populates="quote", cascade="all, delete-orphan"
    )
...
class LineItemORM(Base):
    ...
    name_hu: Mapped[str]
    category_key: Mapped[Optional[str]] = mapped_column(
        ForeignKey("work_categories.key")
    )
