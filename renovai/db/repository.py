...
from ..ingestion.models import RenovationQuote
from .models import (
    BuyerPurchase,
    CPIRecord,
    GeneralNote,
...
class QuoteRepository:
    async def upsert_quote(
        self, session: AsyncSession, parsed_quote: RenovationQuote
    ) -> Quote:
        ...
        stmt = (
            select(Quote)
            .where(Quote.file_name == meta.file_name)
            .options(
        ...
        for item in parsed_quote.line_items + parsed_quote.alternatives:
            if (
                "kohósalak" in (item.notes or "").lower()
                or "kohósalak" in item.name.lower()
            ):
                has_slag = True
    ...
    async def get_quote_by_file(
        self, session: AsyncSession, file_name: str
    ) -> Optional[Quote]:
        stmt = (
            select(Quote)
            .where(Quote.file_name == file_name)
            .options(
                selectinload(Quote.line_items),
                selectinload(Quote.not_included),
                selectinload(Quote.buyer_purchases),
    ...
    async def get_cost_stats(
        ...
        stmt = select(
            ...
            func.avg(
                cast(Quote.total_labor_huf, Float) / cast(Quote.grand_total_huf, Float)
            ),
        )
...
class CPIRepository:
        ...
        for rec in records:
            ...
            if not comp:
                raise AttributeError(
                    "CPI record missing 'component' attribute and no override provided."
                )

            stmt = select(CPIRecord).where(
                and_(
    ...
    async def get_index_series(
        self, session: AsyncSession, component: str
    ) -> List[CPIRecord]:
        stmt = (
            select(CPIRecord)
            .where(CPIRecord.component == component)
            .order_by(CPIRecord.year, CPIRecord.quarter)
        )
        res = await session.execute(stmt)
        return res.scalars().all()
