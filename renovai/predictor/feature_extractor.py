...
def extract_features(quote_json_path: Path) -> QuoteFeatures:
    ...
    for adj_item in all_items:
        ...
        for cat, keywords in CATEGORY_KEYWORDS.items():
            if any(k in name_lower for k in keywords):
                ...
                matched = True
                if cat == "plumbing_heating":
                    has_plumbing = True
                if cat == "electrical":
                    has_electrical = True
                if cat == "flooring" or cat == "tiling":
                    has_flooring = True
                if cat == "demolition":
                    has_demolition = True
    ...
    return QuoteFeatures(
        ...
        demolition_cost_share=cat_costs["demolition"] / grand_total
        if grand_total > 0
        else 0,
        plumbing_cost_share=cat_costs["plumbing_heating"] / grand_total
        if grand_total > 0
        else 0,
...
def apartment_input_to_features(inp: ApartmentInput) -> QuoteFeatures:
    ...
    num_items = 5
    if inp.needs_plumbing:
        num_items += 2
    if inp.needs_electrical:
        num_items += 2
    if inp.needs_flooring:
        num_items += 2
    if inp.needs_full_demolition:
        num_items += 2
