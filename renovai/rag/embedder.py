...
def _embed_with_retry(client: genai.Client, model: str, content: List[str], task_type: str):
...
def embed_chunks(
    ...
    logger.info(f"Retrieved {len(results)} chunks from cache. Need to embed {len(to_embed_indices)} new chunks.")
