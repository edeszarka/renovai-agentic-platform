    ...
    for base_model_id in models_to_try:
        ...
        try:
            ...
            response = _generate_with_smart_retry(
                client=client, model=model_id, contents=contents, config=gen_config
            )
            ...
            if response.candidates and len(response.candidates) > 0:
                ...
                if finish_reason != "STOP":
                    logger.warning(
                        f"Model {model_id} finished with non-standard reason: {finish_reason}"
                    )

            if response.usage_metadata:
                logger.info(
                    f"Success with {model_id}. Tokens: {response.usage_metadata.total_token_count}"
                )
            ...
            logger.warning(
                f"Model {model_id} failed or exhausted. Trying next fallback... Error: {e}"
            )
            last_exception = e
            continue
...
"""
    ...
    response = await client.aio.models.generate_content(
        model=config.model_name, contents=contents, config=gen_config, stream=True
    )
