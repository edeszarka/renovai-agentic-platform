...
def retrieve(
    ...
    semantic_results = {
        c.chunk_id: (c, max(0.0, 1.0 - dist)) for c, dist in semantic_tuples
    }
    ...
    # 4. Filter and Sort
    final_list = [
        v for v in merged_results.values() if v[1] >= config.min_score_threshold
    ]
    ...
    return RetrievedContext(
        chunks=top_chunks, query=query, retrieval_metadata=retrieval_metadata
    )
