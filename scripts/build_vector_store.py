...
from renovai.rag.chunker import chunk_all, count_tokens
...
def main():
    parser = argparse.ArgumentParser(description="RenovAI Vector Store Builder — Module 4")
    parser.add_argument("--quotes-md-dir", type=str, default="data/processed/quotes_md/", help="Quotes MD directory")
    ...
    # 1. Chunking
    console.print("\n[bold yellow]Step 1: Chunking Markdown Files...[/bold yellow]")
    chunks = chunk_all(quotes_dir)
    if not chunks:
        logger.warning("No chunks generated. Check if quotes_md/ contains markdown files.")
        sys.exit(0)
    ...
    # 2. Embedding
    console.print("\n[bold yellow]Step 2: Embedding Chunks via Google GenAI...[/bold yellow]")
    ...
    console.print("\n[bold yellow]Step 3: Storing Chunks in ChromaDB...[/bold yellow]")
    ...
    # 4. Running sample queries to validate
    console.print("\n[bold yellow]Step 4: Running Validation Queries...[/bold yellow]")
    sample_queries = [
    ...
    for idx, q_text in enumerate(sample_queries):
        ...
        try:
            ...
            for rank, (chunk, dist) in enumerate(results):
                console.print(f"  [bold]Rank {rank+1}:[/bold] {chunk.source_file} | Section: {chunk.section} | Dist: {dist:.3f}")
    ...
    console.print("\n[bold yellow]Step 5: Final Vector Store Statistics[/bold yellow]")
