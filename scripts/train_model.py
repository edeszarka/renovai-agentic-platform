...
def main():
    parser = argparse.ArgumentParser(
        description="RenovAI Price Model Trainer — Module 6"
    )
    parser.add_argument(
        "--quotes-dir",
        type=str,
        default="data/processed/quotes_json/",
        help="Adjusted JSONs directory",
    )
    parser.add_argument(
        "--model-dir",
        type=str,
    ...
    if not quotes_dir.exists():
        console.print(
            f"[bold red]Error: Quotes directory not found: {quotes_dir}[/bold red]"
        )
        sys.exit(1)
    ...
    if len(X) == 0:
        console.print(
            "[bold red]Error: No adjusted quotes found to train on. Run inflation adjustment first.[/bold red]"
        )
        sys.exit(1)
    ...
    for model_name, mape in results["mape_comparison"].items():
        status = (
            ...
            if model_name == results["recommended_model"]
            else ""
        )
    ...
    for i, ex in enumerate(results["examples"]):
        ...
        console.print(
            f"  {i + 1}. Actual: [blue]{ex['actual']:,.0f} Ft[/blue] | Pred: [green]{ex['pred']:,.0f} Ft[/green] | Diff: {diff_pct:+.1f}%"
        )
