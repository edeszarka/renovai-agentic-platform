...
def main():
    parser = argparse.ArgumentParser(description="RenovAI RAG Interactive CLI — Module 5")
    ...
    while True:
        try:
            ...
            with console.status("[bold green]Gondolkodom...[/bold green]"):
                if current_debug:
                    response, trace = pipeline.query_with_trace(user_input, conversation_history=history)
                    ...
                    for i, chunk in enumerate(trace.chunks):
                        score = trace.retrieval_metadata["top_scores"][i] if i < len(trace.retrieval_metadata["top_scores"]) else 0
                        console.print(f"[bold]{i+1}. {chunk.source_file} | {chunk.section}[/bold] (Score: {score:.3f})")
