...
def main():
    parser = argparse.ArgumentParser(
        description="RenovAI Pre-Purchase Advisor CLI — Module 7"
    )
    parser.add_argument(
        "--chroma-dir", type=str, default="data/chroma_db/", help="ChromaDB path"
    )
    parser.add_argument(
        "--model-dir", type=str, default="data/models/", help="Price model directory"
    )
    parser.add_argument(
    ...
    if not args.interactive:
        console.print(
            "[yellow]Jelenleg csak az interaktív mód támogatott. Használja a --interactive kapcsolót.[/yellow]"
        )
        return
    ...
    issues_str = Prompt.ask(
        "Vannak ismert hibák? (vesszővel elválasztva, pl. beázás, penész)", default=""
    )
    ...
    try:
        ...
        report_path = Path(
            f"data/reports/report_{timestamp}_{district}ker_{int(area)}sqm.md"
        )
        ...
        console.print(
            f"[bold red]Hiba történt a jelentés generálása közben: {e}[/bold red]"
        )
        import traceback
