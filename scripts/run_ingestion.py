...
from renovai.ingestion.md_writer import write_markdown, format_huf
...
async def async_main():
    parser = argparse.ArgumentParser(description="RenovAI Ingestion Runner — Module 1 Reloaded")
    parser.add_argument("--quotes-dir", type=str, default="data/raw/quotes/", help="Path to raw XLSX quotes")
    parser.add_argument("--json-dir", type=str, default="data/processed/quotes_json/", help="Output path for JSON")
    parser.add_argument("--md-dir", type=str, default="data/processed/quotes_md/", help="Output path for Markdown")
    parser.add_argument("--adjust-inflation", action="store_true", help="Enable inflation calculator adjustments")
    parser.add_argument("--target-date", type=str, default=None, help="Target date for inflation adjustment (YYYY-MM-DD)")
    parser.add_argument("--materials-cpi", type=str, default="data/raw/inflation/materials_cpi.csv", help="Path to materials CPI csv")
    parser.add_argument("--labor-cpi", type=str, default="data/raw/inflation/labor_cpi.csv", help="Path to labor CPI csv")
    ...
    if not args.skip_db:
        ...
        if not db_url:
            logger.error("DATABASE_URL not set in .env. Use --skip-db if database is not needed.")
            return
    ...
    if args.adjust_inflation:
        ...
        if not materials_csv.exists() or not labor_csv.exists():
            logger.error(f"CPI CSV files not found. Ensure {materials_csv} and {labor_csv} exist.")
            return
    ...
    error_count = 0
    total_huf_sum = 0
    style_counts = {"text_only": 0, "standard_5col": 0, "multi_phase": 0, "scope_only": 0}
    districts_seen = set()
    failed_quotes = []
    ...
    stats_table.add_row("Districts Seen", ", ".join(map(str, sorted(list(districts_seen)))))
