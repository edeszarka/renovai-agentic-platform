...
def test_gather_advisory_context(mock_profile):
    ...
    chunk = Chunk(
        chunk_id="c1", source_file="f1.md", source_type="quote", 
        section="Sec", content="Content", metadata={}
    )
    mock_resp = RAGResponse(
        answer="Ans", sources_cited=["f1.md"], confidence="low", 
        retrieval_metadata={"top_scores": [0.9]}, tokens_used=10
    )
...
def test_overall_risk_logic():
    ...
    def check_risk(items):
        risk = "alacsony"
        if any(it.priority == "kritikus" for it in items): return "magas"
        elif items: return "közepes"
        return risk
...
def test_render_report_md(mock_profile):
    report = AdvisoryReport(
        ...
        questions_for_seller=[ChecklistItem(category="Víz", item="Strang csere?", priority="kritikus", why="Fontos")],
        inspection_checklist=[],
        red_flags=[],
        cost_estimate={"estimate_low_huf": 1000, "estimate_mid_huf": 2000, "estimate_high_huf": 3000, "inflation_adjusted_to": "2024-01-01"},
        similar_cases=[],
...
def test_generate_report_end_to_end(mock_gather, mock_similar, mock_predict, mock_client_class, mock_profile):
    mock_gather.return_value = ("Context", ["f1.md"])
    mock_predict.return_value = {"estimate_low_huf": 1, "estimate_mid_huf": 2, "estimate_high_huf": 3, "inflation_adjusted_to": "2024"}
    ...
    mock_resp_json.text = json.dumps({
        "questions": [{"category": "V", "item": "Q", "priority": "fontos", "why": "W"}],
        "inspection": [],
        "red_flags": [{"category": "S", "item": "F", "priority": "fontos", "why": "W"}]
    })
    ...
    report = generate_report(mock_profile, pipeline, Path("models"), MagicMock(), gem_cfg)
