...
from renovai.rag.gemini_client import GeminiConfig, RAGResponse, call_gemini
from renovai.rag.pipeline import RAGPipeline
from renovai.rag.retriever import (
    RetrievalConfig,
    RetrievedContext,
    assemble_context,
    retrieve,
...
def mock_chunks():
    return [
        ...
        Chunk(
            ...
            source_type="quote",
            section="Sec3",
            content="Content of chunk 3 is very long for testing truncation logic. "
            * 10,
...
def test_hybrid_retrieval_logic(mock_chunks):
    ...
    mock_vs.query.return_value = [
        (mock_chunks[0], 0.2),
        (mock_chunks[1], 0.4),
    ]  # dist 0.2 -> score 0.8, dist 0.4 -> score 0.6
...
def test_assemble_context_truncation(mock_chunks):
    retrieved = RetrievedContext(
        chunks=mock_chunks, query="query", retrieval_metadata={}
    )
...
def test_citation_and_confidence():
    mock_answer = (
        "A fürdőszoba felújítás drága [FORRÁS 1]. De megéri [FORRÁS 2] és [FORRÁS 4]."
    )
