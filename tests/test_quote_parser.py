...
from renovai.ingestion.quote_parser import infer_quote_year_from_path, parse_quote
...
def test_address_extractor_no_floor_clause():
    addr = extract_address(
        "1024 Rózsahegy utca 4. komplett, 1970-es évek, nincs salak, 83nm"
    )
...
@pytest.mark.skipif(
    not (FIXTURE_DIR / "Árajánlat_-_1081_Népszínház_utca_29__1__emelet.xlsx").exists(),
...
@pytest.mark.skipif(
    not (
        FIXTURE_DIR / "Árajánlat_-_1126_Hollósy_Simon_utca_30__2__emelet.xlsx"
    ).exists(),
    reason="Fixture missing",
)
def test_hollosy():
...
@pytest.mark.skipif(
    not (FIXTURE_DIR / "Árajánlat__2_.xlsx").exists(), reason="Fixture missing"
)
def test_fazisok():
...
@pytest.mark.skipif(
    not (FIXTURE_DIR / "Árajánlat_-_komplett_munkák.xlsx").exists(),
    reason="Fixture missing",
)
def test_komplett():
...
def test_parse_quote_handles_read_only_unpadded_trailing_rows():
    ...
    """
    src = Path(
        "data/raw/quotes/2025/1015 Csalogány utca 12. fsz majdnem komplett, 1930-as évek, van salak, 32nm.xlsx"
    )
    if not src.exists():
