...
from renovai.ingestion.models import LineItem, QuoteMetadata, RenovationQuote
...
def test_graceful_handling_earliest_date(sample_price_index, caplog):
    ...
    expected_to_val = (
        get_factor(sample_price_index, "materials", date(2023, 2, 15), to_date) * 0.900
    )
    expected_factor = expected_to_val / 0.900

    assert pytest.approx(factor_mat) == expected_factor
    assert any(
        "before earliest materials record" in record.message
...
def test_adjust_quote(sample_price_index):
    ...
    factor_labor = get_factor(
        sample_price_index, "labor", quote.metadata.quote_date, target_date
    )
    factor_materials = get_factor(
        sample_price_index, "materials", quote.metadata.quote_date, target_date
    )
...
def test_compound_inflation_factor_basic(sample_price_index):
    factor = compound_inflation_factor(
        2023, date(2024, 8, 15), sample_price_index, "materials"
    )
    assert factor > 1.0
...
def test_compound_inflation_factor_before_data_clamps(sample_price_index, caplog):
    with caplog.at_level(logging.WARNING):
        factor = compound_inflation_factor(
            2022, date(2024, 2, 15), sample_price_index, "materials"
        )
...
def test_inflate_quote_value_same_year_near_one(sample_price_index):
    val = inflate_quote_value(
        50000, date.today().year, date.today(), sample_price_index
    )
