...
from .errors import logging_middleware, setup_exception_handlers
from .schemas import (
    AdvisoryRequest,
    AdvisoryResponse,
    EstimateRequest,
...
async def lifespan(app: FastAPI):
    ...
    if not Path(config.chroma_dir).exists():
        raise FileNotFoundError(
            f"ChromaDB directory not found at {config.chroma_dir}. Run ingestion first."
        )

    if not Path(config.models_dir).exists():
        raise FileNotFoundError(
            f"Models directory not found at {config.models_dir}. Run training first."
        )
    ...
    app.state.price_index = load_price_index(
        Path(config.inflation_materials_csv), Path(config.inflation_labor_csv)
    )
    ...
    if model_path.exists():
        ...
        logger.warning(
            f"Price model not found at {model_path}. Predict endpoint will fail."
        )
...
async def get_query_stream(question: str, district: Optional[int] = None):
    meta_filter = None
    if district:
    ...
    async def event_generator():
        async for token in app.state.rag_pipeline.stream_query(
            question=question, metadata_filter=meta_filter
        ):
            yield f"data: {token}\n\n"
        yield "data: [DONE]\n\n"
...
async def post_ingest(request: IngestRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_ingestion_task, task_id, request.quotes_dir, request.adjust_inflation
    )
